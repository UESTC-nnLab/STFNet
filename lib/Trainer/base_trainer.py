...
class BaseTrainer(object):
    ...
    def run_eval_epoch(self, phase, epoch, data_loader, base_s, dataset):
        ...
        for iter_id, (im_id, batch) in enumerate(data_loader):
            ...
            dets = post_process(output, meta, num_classes = dataset.num_classes)
            # ret = merge_outputs([dets], num_classes=3, max_per_image=opt.K)
            ret = merge_outputs([dets], num_classes=dataset.num_classes, max_per_image=opt.K)
            # results[im_id.numpy().astype(np.int32)[0]] = ret
            results[np.array(im_id,dtype=np.int32)[0]] = ret
